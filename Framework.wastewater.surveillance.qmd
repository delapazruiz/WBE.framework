...
tm_shape(blocks_tbl_f['wwtp.conex']) + #tm_polygons() + 
  ...
  tm_layout(main.title.position=c("left","top"),
            main.title=
              "Early outbreak, March 21, 9-10hrs, 06 min",
            legend.outside.position= "right",
            outer.margins=0,inner.margins=0,between.margin=0)
```


#### Figure 6.b Mapserie at 11:00
...
tm_shape(blocks_tbl_f['wwtp.conex']) + #tm_polygons() + 
  ...
  tm_layout(main.title.position=c("left","top"),
            main.title=
              "Early outbreak, March 21, 11-12hrs, 06 min",
            legend.outside.position= "right",
            outer.margins=0,inner.margins=0,between.margin=0)
```
...
### Figure 9. Surveillance: WWTP and Maintinance hole
...
covid.mh460.peak2 <- manholes.snt.pol|> 
    filter(manhole.id == 460&
             event.typ=="poo"&
             date.time <= as.POSIXct("2022-04-01 17:00:00", tz='UTC') &
            date.time >= as.POSIXct("2022-04-01 07:00:00", tz='UTC'))

covid.mh460.peak2 <- covid.mh460.peak2|>
    filter(virus == "Infected"|virus=="Recovered")|>
  mutate(inf.stool=1)

covid.mh460.peak2$date.time <- align.time(
  as.POSIXct(covid.mh460.peak2$date.time), 12 * 60) # temporal res in min=6

covid.mh460.peak2 <- aggregate(
  inf.stool ~format(as.POSIXct(covid.mh460.peak2$date.time), "%Y-%m-%d %H:%M")+
    covid.mh460.peak2$manhole.id,
  data= covid.mh460.peak2,
  FUN = sum)%>% as_tibble()

colnames(covid.mh460.peak2) <-c(
  "date.time","manhole.id","stool.infected")

covid.mh460.peak2<- covid.mh460.peak2[,-2]

covid.mh460.peak2$date.time <- parse_date_time(
  covid.mh460.peak2$date.time, orders = c("%Y-%m-%d %H:%M"))

covid.mh460.peak3 <- manholes.snt.pol|>
  filter( manhole.id == 460&
            virus=="Infected"&
            event.typ=="poo"&
            date.time <= as.POSIXct("2022-04-01 17:00:00", tz='UTC') &
...
covid.mh460.peak3$date.time <- align.time(
  as.POSIXct(covid.mh460.peak3$date.time), 12 * 60) # temporal res in min=6

covid.mh460.peak3 <- aggregate(
  inf.stool ~format(as.POSIXct(covid.mh460.peak3$date.time), "%Y-%m-%d %H:%M")+
    covid.mh460.peak3$manhole.id,
  data= covid.mh460.peak3,
  FUN = sum)%>% as_tibble()

colnames(covid.mh460.peak3) <-c(
  "date.time","manhole.id","stool.infected")

covid.mh460.peak3<- covid.mh460.peak3[,-2]

covid.mh460.peak3$date.time <- parse_date_time(
  covid.mh460.peak3$date.time, orders = c("%Y-%m-%d %H:%M"))

 # covid.mh460.peak2
 # covid.mh460.peak3

covid.mh460.peak4<- covid.mh460.peak3 |> left_join(covid.mh460.peak2,by = "date.time")

colnames(covid.mh460.peak4) <-c(
...
dygraph(covid.mh460.peak4,
  ...
  dySeries('Infected', color='#756bb1')|> 
  dySeries('Infected+recovered', color='#31a354')
...
covid.mh258.peak2 <- manholes.snt.pol|> 
    filter(manhole.id == 258&
             event.typ=="poo"&
             date.time <= as.POSIXct("2022-04-01 17:00:00", tz='UTC') &
            date.time >= as.POSIXct("2022-04-01 07:00:00", tz='UTC'))

covid.mh258.peak2 <- covid.mh258.peak2|>
    filter(virus == "Infected"|virus=="Recovered")|>
  mutate(inf.stool=1)

covid.mh258.peak2$date.time <- align.time(
  as.POSIXct(covid.mh258.peak2$date.time), 12 * 60) # temporal res in min=6

covid.mh258.peak2 <- aggregate(
  inf.stool ~format(as.POSIXct(covid.mh258.peak2$date.time), "%Y-%m-%d %H:%M")+
    covid.mh258.peak2$manhole.id,
  data= covid.mh258.peak2,
  FUN = sum)%>% as_tibble()

colnames(covid.mh258.peak2) <-c(
  "date.time","manhole.id","stool.infected")

covid.mh258.peak2<- covid.mh258.peak2[,-2]

covid.mh258.peak2$date.time <- parse_date_time(
  covid.mh258.peak2$date.time, orders = c("%Y-%m-%d %H:%M"))

covid.mh258.peak3 <- manholes.snt.pol|>
  filter( manhole.id == 258&
            virus=="Infected"&
            event.typ=="poo"&
            date.time <= as.POSIXct("2022-04-01 17:00:00", tz='UTC') &
...
covid.mh258.peak3$date.time <- align.time(
  as.POSIXct(covid.mh258.peak3$date.time), 12 * 60) # temporal res in min=6

covid.mh258.peak3 <- aggregate(
  inf.stool ~format(as.POSIXct(covid.mh258.peak3$date.time), "%Y-%m-%d %H:%M")+
    covid.mh258.peak3$manhole.id,
  data= covid.mh258.peak3,
  FUN = sum)%>% as_tibble()

colnames(covid.mh258.peak3) <-c(
  "date.time","manhole.id","stool.infected")

covid.mh258.peak3<- covid.mh258.peak3[,-2]

covid.mh258.peak3$date.time <- parse_date_time(
  covid.mh258.peak3$date.time, orders = c("%Y-%m-%d %H:%M"))

 # covid.mh258.peak2
 # covid.mh258.peak3

covid.mh258.peak4<- covid.mh258.peak3 |> left_join(covid.mh258.peak2,by = "date.time")

colnames(covid.mh258.peak4) <-c(
  "date.time","Infected","Infected+recovered")
